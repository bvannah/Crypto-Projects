...
def singleByteXORCipher(inputBytes): #inputs an array of ints, outputs a tuple of (key, message)
    highAscii = [] 
    for byte in range(256): #for each possible byte
        testStr = decryptUsingByte(inputBytes, byte)
        if(evaluateStr(testStr)>.20): #if a string is more than 80% ascii, 
            highAscii.append((testStr, byte, evaluateStr(testStr))) #then it is good enough to look at
    highAscii.sort(key=evaluateStrTuple, reverse=True)
    if(len(highAscii) == 0): #if we didn't find anything with more than 80% Ascii
        return ("No Key :(", "")
    return (highAscii[0][1], highAscii[0][0], highAscii[0][2])
            
def main():
    testString = strToBytes("1b37373331363f78151b7f2b783431333d78397828372d363c78373e783a393b3736")
